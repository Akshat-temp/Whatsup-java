...
public class GroupDAO {
    ...
    public static boolean addMember(int groupId, String phone) {
        String sql = "INSERT INTO group_members (group_id, phone) VALUES (?, ?)";
        try (Connection c = DBConnection.getConnection();
    ...
    public static List<String> getMemberPhones(int groupId) {
        ...
        String sql = "SELECT phone FROM group_members WHERE group_id = ?";
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, groupId);
            ResultSet rs = ps.executeQuery();
    ...
    public static boolean isMember(int groupId, String phone) {
        String sql = "SELECT 1 FROM group_members WHERE group_id = ? AND phone = ?";
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, groupId);
            ps.setString(2, phone);
